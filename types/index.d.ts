...
declare var Media: {
	new (src: string, mediaSuccess: () => void, mediaError?: (error: MediaError) => any, mediaStatus?: (status: number) => void): Media;
	//Media statuses
	MEDIA_NONE: number;
...
interface Media {
	 ...
	 */
	getCurrentAmplitude(mediaSuccess: (amplitude: number) => void, mediaError?: (error: MediaError) => void): void;
	/**
	 ...
	 * @param rate The rate to set for playback. The value must be within the range of 0.5 to 2.0.
	 */
	setRate(rate: number): void;

	/**
	 * Set the pan for an audio file.
	 * @param pan The pan to set for playback. The value must be within the range of -1.0 to 1.0.
	 */
